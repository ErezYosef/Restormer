...
def get_diffusion(name):
    ...
    elif name.startswith('dvir'):
        if 'freeze' in name:
            return concat_diffusion.BaseDiffusion_dvir_freeze
        return concat_diffusion.BaseDiffusion_dvir
    elif name == 'condfull':
...
'''
def create_model_wrap(model_class, **kwargs):
    model = create_model_new(model_class=model_class, **kwargs)
    model.separable_model = SepNet_mult_colors_e('True_randn', return_rgb=False)
    model.separable_model.load_state_dict(torch.load('data/sepnet_e0222_1708.pt')['model'])
    print('init separable_model add to model')
    if kwargs['diffusion_type'] == 'soft':
        model.flatnet = FlatNet_orig(4)
        model.flatnet.load_state_dict(
            torch.load('/data/erez/flatnet/flatnet_separable_pointGrey_randomInit', map_location=torch.device('cpu')))
        print('init soft flatnet model added to model')
    return model
'''
