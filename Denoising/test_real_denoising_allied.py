...
# CUDA_VISIBLE_DEVICES=3 python test_real_denoising_allied.py --weights basicsr/experiments/RealDenoisingAlliedlr1e4_Restormer/models/net_g_200.pth
...
clean_w_path = args.weights.strip("./")
args.result_dir = os.path.join(args.result_dir, 'finetune', clean_w_path.split('.pt')[0]+'')
...
args.config_file = 'diffusion/coco_configs/allied_concat_sample_config.yaml'
...
dict_args['fpath_noisy'] = 't100gain30.88'
...
for sample_condition_data in val_loader:
    ...
    restored_patch = torch.clamp(restored_patch, 0, 1).cpu().detach().squeeze(0)
    ...
    save_img(tensor2img(gt_rgb, min_max=(0,1)), gt_path)
    save_img(tensor2img(lq_rgb, min_max=(0,1)), low_res_path)
    save_img(tensor2img(restored_patch, min_max=(0,1)), sample_path)
    torch.save(gt_rgb, gt_path.replace('.png', '.pt'))
    torch.save(lq_rgb, low_res_path.replace('.png', '.pt'))
    torch.save(restored_patch, sample_path.replace('.png', '.pt'))
