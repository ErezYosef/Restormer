...
def generate_train():
    src = '/data1/erez/datasets/sidd/srgb_full'
    tar = '/data1/erez/datasets/sidd/srgb_crops' # 'Datasets/train/SIDD'
    ...
    # files2 = natsorted(glob(os.path.join(src, '*', '*', '*.PNG')))
    # files.extend(files2)
    ...
    patch_size = 256
    overlap = -256 # to get subsampling
    p_max = 0
        ...
        if w > p_max and h > p_max:
            ...
            for i in w1:
                for j in h1:
                    ...
                    lr_savename = os.path.join(lr_tar, filename + '_' + str(num_patch) + '.png')
                    hr_savename = os.path.join(hr_tar, filename + '_' + str(num_patch) + '.png')
...
def generete_val():
    src = '/data1/erez/datasets/sidd/srgb_full'
    tar = '/data1/erez/datasets/sidd/srgb_crops_val'  # 'Datasets/train/SIDD'
    ...
    patch_size = 256
    overlap = -2256  # to get subsampling
    p_max = 0
    ...
    def save_files(file_):
        ...
        # w1.append(w-patch_size)
        # h1.append(h-patch_size)
        w1 = random.choices(w1, k=1)
        h1 = random.choices(h1, k=1)
        for i in w1:
            for j in h1:
                num_patch += 1
