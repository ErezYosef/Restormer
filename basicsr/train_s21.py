...
def init_loggers(opt):
    time_name = f"train_{opt['name']}_{get_time_str()}"
    ...
    if opt['logger'].get('use_wandb_logger') and 'debug' not in opt['name']:
        import wandb
        wandb_dir = opt['logger'].get('use_wandb_logger')
        os.makedirs(wandb_dir, exist_ok=True)
        wandb.init(project='Restormer', entity='cpil', name=time_name, job_type='train', dir=wandb_dir)
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if current_iter % opt['logger']['print_freq'] == 0:
                ...
                images_dict = {'lq': lq, 'gt': gt, 'preds': model.preds}
                msg_logger(log_vars, images_dict)
            ...
            if opt.get('val') is not None and (current_iter %
                ...
                log_vars.update(model.metric_results)
